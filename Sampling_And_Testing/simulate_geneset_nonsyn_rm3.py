...
v = pd.read_csv("AccelEvol_Missense.txt", sep = "\t")

#THIS VERSION
#Does it without the top n genes for each species to verify that a category isn't solely driven by RET, TTN etc.
n = 3
out_h = pd.DataFrame()
out_c = pd.DataFrame()
tot_h = 0
for key in d_GS.keys():
    ...
    #Removing top n genes
    vv = v[v["3"].isin(genes)]
    toss_n_h = list(vv.sort_values("Hum Sum PhyloP", ascending = False)["3"])[0:n]
    toss_n_c = list(vv.sort_values("Chp Sum PhyloP", ascending = False)["3"])[0:n]
    toss = toss_n_h + toss_n_c
    genes_new = [g if g not in toss else "" for g in genes]
    hum_s = pd.DataFrame(hum[hum[3].isin(np.intersect1d(genes_new, df.index))])
    chp_s = pd.DataFrame(chp[chp[3].isin(np.intersect1d(genes_new, df.index))])
...
for j in range(900000, 1100000):
    ...
    if c % 100000 == 0 and c != 900000:
        new_sum.to_csv("AccelEvol_Missense_" + geneset + "_Sim_rm3_" + str(c) + ".txt", sep = "\t")
        print(c)
    for i in aa:
